...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case mine:
      return mineEffect(choice1, choice2, handPos, currentPlayer, state);
    ...
    case minion:
      return minionEffect(choice1, choice2, handPos, currentPlayer, state);
    ...
    case tribute:
      return tributeEffect(tributeRevealedCards, currentPlayer, nextPlayer, state);
...
int minionEffect(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)
{
      ...
      //discard card from hand, ADDED BUG changed 0 to 1, the card will now go to Trashed instead of played, meaning won't go back to Deck
      discardCard(handPos, currentPlayer, state, 1);
      ...
      else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
	  ...
	  while(numHandCards(state) > 0)
	    {
	      discardCard(handPos, currentPlayer, state, 1);//discard card from hand, ADDED BUG changed 0 to 1, the card will now go to Trashed instead of played, meaning won't go back to Deck
	  ...
	  for (i = 0; i < state->numPlayers; i++)
	    {
	      if (i != currentPlayer)
		{
		  if ( state->handCount[i] >= 4 )// ADDED bug, changed
		    {
		      //discard hand
...
int ambassadorEffect(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)
{
	...
	printf("Player %d reveals card number: %d\n", currentPlayer, state->hand[currentPlayer][choice1]);
      ...
      //each other player gains a copy of revealed card, ADDED BUG, removed i =0, so now the other players don't get a copy of revealed card
      for (; i < state->numPlayers; i++)
	{
      ...
      //discard played card from hand, ADDED BUG, 0 to 1, now the card goes from discard to trash
      discardCard(handPos, currentPlayer, state, 1);
...
int tributeEffect(int tributeRevealedCards[], int currentPlayer, int nextPlayer, struct gameState *state)
{
	...
	if (state->deckCount[nextPlayer] == 0){
	  ...
	  //shuffle(nextPlayer,state);//Shuffle the deck, ADDED BUG, remove shuffle, so now there will be no more deck once it reaches 0
	}
	tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
	...
	tributeRevealedCards[1] = -1;
      ...
      for (i = 0; i <= 2; i ++){
	if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver ){//Treasure cards, ADDED bug got rid of tributeRevealedCards[i] == gold, so gold will no longer add +2 coins
	  state->coins += 2;
	}
...
int mineEffect(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)
{
      ...
      if ( (getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2) )
	{
      ...
      //ADDED Bug, 2 -> 1, now the card will go to the deck instead of the players hand
      gainCard(choice2, state, 1, currentPlayer);

      //discard card from hand, ADDED BUG, 0->1, now the card will be trashed
      discardCard(handPos, currentPlayer, state, 1);
